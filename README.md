...
| POST   | /api/categories | Create new category (By logged in user) | [JSON](#categorycreate) |
| PUT    | /api/categories/{id} | Update category (If category belongs to logged in user or logged in user is admin) | [JSON](#postupdate) |
| DELETE | /api/categories/{id} | Delete category (If category belongs to logged in user or logged in user is admin) | |
...
##### <a id="categoryupdate">Update Category -> /api/categories</a>
```json
{
	"name": "sunt aut facere repellat provident occaecati excepturi optio reprehenderit"
}
